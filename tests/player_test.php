 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class media_wmp_testcase extends advanced_testcase {
    ...
    public function test_is_installed() {
        ...
        $this->assertEquals(['wmp' => 'wmp'], $sortorder);
    }
    ...
    public function test_embed_url() {
        ...
        $url = new moodle_url('http://example.org/1.avi');
    ...
    public function test_embed_link() {
        global $CFG;
        $url = new moodle_url('http://example.org/some_filename.avi');
    ...
    public function test_embed_media() {
        ...
        $this->assertRegExp('~mediaplugin_wmp~', $content);
        $this->assertRegExp('~</object>~', $content);
        $this->assertRegExp('~width="123" height="99"~', $content);
        $this->assertContains('6BF52A52-394A-11d3-B153-00C04F79FAA6', $content);
    }
}
